...
export default (event, context, callback) => {
	const key = process.env.signInKey
	jwt.verify(event.authorizationToken, key, { ignoreExpiration: false }, async (err, decoded) => {
		if (err) {
			logger.error(`Error decoding JWT: ${err}`)
			return context.fail('Unauthorized')
		} else {
			try {

				const principalId = decoded.user.companyId
				const apiOptions = {}
				...
				logger.error(`Error in authoriser: ${e.message}`)
				return context.fail('Unauthorized')
			}
		}
	})
